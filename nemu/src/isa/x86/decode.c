...
void read_ModR_M(DecodeExecState *s, Operand *rm, bool load_rm_val, Operand *reg, bool load_reg_val) {
  ...
  s->isa.ext_opcode = m.opcode; // save extended opcode
  if (reg != NULL) operand_reg(s, reg, load_reg_val, m.reg, reg->width); // save reg
  if (m.mod == 3) operand_reg(s, rm, load_rm_val, m.R_M, rm->width); // if mod is 3, save reg
  else {  // else, 
    load_addr(s, &m, rm); // load mem address
    if (load_rm_val) rtl_lm(s, &rm->val, s->isa.mbase, s->isa.moff, rm->width); // load_val mem
    rm->preg = &rm->val;
  }
}
...
void read_ModR_M_C(DecodeExecState *s, Operand *rm, Operand *reg){
  ModR_M m;
  m.val = instr_fetch(&s->seq_pc, 1);
  assert(m.mod == 3 && rm->width == 4);
  operand_reg(s, rm, true, m.R_M, rm->width);
